...
class RemoveTediousQuestConditions implements IPostDBLoadMod {
  ...
  public postDBLoad(container: DependencyContainer): void {
    ...
    const quests = this.container
      .resolve<DatabaseServer>("DatabaseServer")
      .getTables().templates.quests;
      ...
      if (this.config[eachOption] != false) {
        switch (eachOption) {
          ...
          case "setPMC":
            ...
            ].conditions.AvailableForFinish[0].counter.conditions[0].target =
              "AnyPmc";
            break;
  ...
  private findCounterCreators(target: string, quests: any): void {
    ...
    for (let eachQuest in quests) {
      for (let eachCondition in quests[eachQuest].conditions
        .AvailableForFinish) {
        if (
          quests[eachQuest].conditions.AvailableForFinish[eachCondition]
            .conditionType === "CounterCreator"
        )
          this.removeNested(
            quests[eachQuest].conditions.AvailableForFinish[eachCondition],
            target,
            quests[eachQuest].QuestName
          );
      }
    }
    if (this.config.debug === true && this.loggerBuffer.length > 0)
  ...
  private removeNested(input: any, target: string, questName: string): void {
    ...
    thisCondition
      .filter((finalCondition: any) => finalCondition.conditionType === target)
