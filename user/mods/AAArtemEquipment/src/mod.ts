...
class AddItems implements IPostDBLoadMod
{
    ...
    public addItemToStaticLoot(tables: any, mapName: string | null, itemId: string, probability: number, containers: string[]) {
        if (tables.locations) {
            let locations: { [key: string]: ILocation } = tables.locations;
            Object.keys(locations).forEach((locationKey: string) => {
                if (mapName && mapName.toLowerCase() !== locationKey.toLowerCase()) {
                    return; 
                }

                if (locations[locationKey] && typeof locations[locationKey].staticLoot === 'object') {
                    let staticLoot: any = locations[locationKey].staticLoot;
                    
                    containers.forEach(container => {
                        if (staticLoot[container] && typeof staticLoot[container] === 'object' && Array.isArray(staticLoot[container].itemDistribution)) {
                            staticLoot[container].itemDistribution = staticLoot[container].itemDistribution || [];
                            staticLoot[container].itemDistribution.push({ tpl: itemId, relativeProbability: probability });
                        } else {
                            this.logger.debug(`Error: ${container} does not exist or itemDistribution is not an array.`);
                        }
                    });
                } else {
                    this.logger.debug(`Error: ${locationKey} does not exist or staticLoot is not an object.`);
                }
            });
        } else {
            this.logger.debug('Error: tables.locations is undefined.');
        }
    }
