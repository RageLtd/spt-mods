...
class AddItems implements IPostDBLoadMod
{
    ...
    public postDBLoad(container: DependencyContainer): void
    {
        ...
        //Mod Info
        const modFolderName =   "Carl-300 Blackout Expansion";
        const modFullName =     "Carl-300 Blackout Expansion";
        ...
        //NOT IMPLEMENTED YET
        //this.checkJSON(this.mydb.GW_items);
        //this.checkJSON(this.mydb.GW_clothes);
        ...
        for (const [GWID, GWItem] of Object.entries(this.mydb.GW_items))
        {
            if ( GWItem.enable )
            {
                const sptID = GWItem.sptID;

                //Items + Handbook
                if ( "clone" in GWItem )
                {
                    this.cloneItem(GWItem.clone, GWID, sptID);
                    this.copyToFilters(GWItem.clone, GWID, sptID, GWItem.enableCloneCompats, GWItem.enableCloneConflicts);
                }
                else this.createItem(GWID, sptID);

                //Locales (Languages)
                this.addLocales(GWID, sptID, GWItem);
        ...
        {
            const sptID = GWItem.sptID;
            if ( this.mydb.GW_items[GWID].enable ) this.addToFilters(GWID, sptID);
        }
        ...
        {
            const sptID = GWArticle.sptID;

            //Articles + Handbook
            if ( "clone" in GWArticle )
            {
                this.cloneClothing(GWArticle.clone, GWID, sptID);
            }
            else
            {
                //Doesn't do anything yet...
                this.createClothing(GWID, sptID);
            }

            //Locales (Languages)
            this.addLocales(GWID, sptID, undefined, GWArticle);
    ...
    private cloneItem(itemToClone: string, GWID: string, sptID: string): void
    {
        ...
        this.db.templates.items[sptID] = GWItemOut;
    ...
    private copyToFilters(itemClone: string, GWID: string, sptID: string, enableCompats = true, enableConflicts = true): void
    {
        //Find the original item in all compatible and conflict filters and add the clone to those filters as well
        //Will skip one or both depending on the enable parameters found in GW_items.json (default is true)

        //Get a list of all our custom items so we can skip over them:
        const sptIDs: string[] = [];
        for (const GWItem of Object.values(this.mydb.GW_items)) sptIDs.push(GWItem.sptID)
    ...
    private addToFilters(GWID: string, sptID: string): void
    {
        ...
        //Manually add items into an GW item's filters
        if ( "addToThisItemsFilters" in GWNewItem )
        {
            const   GWItemFilters =      this.getFilters(sptID)[0];
            let     GWConflictingItems = this.getFilters(sptID)[1];
        ...
        //Manually add GW items to pre-existing item filters.
    ...
    private cloneClothing(articleToClone: string, GWID: string, sptID: string): void
    {
        if ( this.mydb.GW_clothes[GWID].enable || !("enable" in this.mydb.GW_clothes[GWID]) )
        {
            ...
            GWClothingOut._id = sptID;
            GWClothingOut._name = sptID;
            GWClothingOut = this.compareAndReplace(GWClothingOut, this.mydb.GW_clothes[GWID]["customization"]);

            //Add the new item to the database
            this.db.templates.customization[sptID] = GWClothingOut;
    ...
    */
    ...
    private addLocales(GWID: string, sptID: string, GWItem?: IGWItem, GWArticle?: IGWCustomizationItem): void
    {
